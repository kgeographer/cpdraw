...
class DrawView(LoginRequiredMixin, View):
  ...
  def get(self, request, *args, **kwargs):
    context = {
      'projid': self.kwargs['projid'],
      'mapbox_token_mb': settings.MAPBOX_TOKEN_MB,
      'mapbox_token_kg': settings.MAPBOX_TOKEN_KG,
    }
...
def deleteFeature(request,*args,**kwargs):
  print('deleteFeature() request', request.POST)
  print('in deleteFeature() kwargs', kwargs)
  if request.method == 'POST':
    _id = int(request.POST.get('lid'))
    print('_id',_id)
    try:
      #get_object_or_404(Feature, jsonb__properties__leaflet_id = _id).delete()
      get_object_or_404(Feature, jsonb__properties__leaflet_id__in=[int(_id),str(_id)]).delete()
      result = {"msg":"deleted "+str(_id)}
    except:
      result = {"msg":"delete of "+str(_id)+' failed: '+str(sys.exc_info())}
